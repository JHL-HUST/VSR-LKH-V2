...
double DDD(double x, double PP){
    return PP*exp(-(log(PP)/((double)(Init_P + 1)))*x);
}
            ...
            if (X4 == 1 && !Forbidden(t4, t1) &&
                ...
                *Gain = G2 - C(t4, t1);
                if (CurrentPenalty != 0 || TSPTW_Makespan || *Gain > 0) {
                    Swap1(t1, t2, t3);
                    int tt = Improvement(Gain, t1, t2);
                    dtemp = (*Gain - *G0 + C(t2, t1)) / Precision;
                    if(PGain*dtemp>=0)
                        ddd = PGain+dtemp;
                    else
                        ddd = PGain;
                    ...
                    if(ddd<0)
                        ddd = -log(2.8-ddd);
                    if(ddd > 0)
                                    ddd*=DDD(CurrentPenalty,PP);
                    Ntt23->Value = Ntt23->Value + Alpha2 * (ddd - Ntt23->Value);
                    if (tt) { 
                        return 0;
                    }
                }
                        ...
                        if (CurrentPenalty != 0 ||
                            ...
                            if (NewPenalty < CurrentPenalty ||
                                ...
                                Make3OptMove(t1, t2, t3, t4, t5, t6,
                                    Case6);
                                int tt = Improvement(Gain, t1, t2);
                                dtemp = (*Gain - G2 + C(t4, t3)) / Precision;
                                if(PGain*dtemp>=0)
                                    ddd = PGain+dtemp;
                                else
                                    ddd = PGain;
                                ...
                                if(ddd > 0)
                                    ddd*=DDD(CurrentPenalty,PP);
                                Ntt45->Value = Ntt45->Value + Alpha2 * (ddd - Ntt45->Value);
                                if (Method == 1)
                                    dtemp = (C(t1, t2) - C(t2, t3)) / Precision + Gama * t4->CandidateSet[0].Value;
                                else if (Method == 2){
                                    for (Candidate* Nttemp = t4->CandidateSet; Nttemp->To; Nttemp++) {
                                        if (Nttemp->To == t5) {
                                            dtemp = (C(t1, t2) - C(t2, t3)) / Precision + Gama * Nttemp->Value; 
                                            break;
                                        }
                                    }
                                }
                    ...
                    if (K < 5) {
                        ...
                        if (!Backtracking || Swaps > 0) {
                            ...
                            while (t = BestSubsequentMove(t1, t, &G, Gain)){
                                dtemp; 
                                if (Method == 1)
                                    dtemp = (C(t3, t4) - C(t4, t5)) / Precision + Gama * t6->CandidateSet[0].Value;
                                else if (Method == 2){
                                    for (Candidate* Nttemp = t6->CandidateSet; Nttemp->To; Nttemp++) {
                                        if (Nttemp->To == NextNode) {
                                            dtemp = (C(t3, t4) - C(t4, t5)) / Precision + Gama * Nttemp->Value; 
                                            break;
                                        }
                                    }
                                }
